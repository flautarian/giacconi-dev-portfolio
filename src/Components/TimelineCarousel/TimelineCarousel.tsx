...
const TimelineCarousel = (props) => {
    ...
    return (
        <div ref={rootRef} className={`${isdark ? 'gradient-dark' : 'gradient-light'}`}>
            {/* MOBILE */}
            <MobileView style={{ height: "100dvh", width: "100dvw" }}>
                <Carousel responsive={responsive} className="size-full carousel" draggable arrows={!isMobile}>
                    {props.values.map((c: any, i: number) => (
                        <div key={`${c.name}-${i}`} className=" m-5 card bg-base-100 drop-shadow-[0px_7px_10px_rgba(0,0,0,1)] certification max-h-[75dvh]" >
                            <div className="card size-full bg-base-200 shadow-xl">
                                <div className="card-body pt-1 flex flex-col">
                                    ...
                                    <div className="pb-2 mt-5 text-xs text-balance hyphens-auto">
                                        ...
                                        <p><span className="text-primary">{t("_service_time")}: </span> {t("_from")} {Utils.formatDate(c.dateFrom)} - {!c.dateTo ? t("_continue_working") : `${t("_to")} ${Utils.formatDate(c.dateTo)}`}</p>
                                    </div>
                                    <hr />
                                    {/* ADVANCED-INFO */}
